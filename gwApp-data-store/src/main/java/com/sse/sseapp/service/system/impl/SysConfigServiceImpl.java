...
import com.sse.sseapp.mapper.system.SysConfigMapper;
...
import static com.sse.sseapp.core.web.domain.AjaxResult.error;
...
public class SysConfigServiceImpl implements ISysConfigService {
    ...
    public int insertConfig(SysConfig config) {
        //设置参数配置UUID    根据SnowUtils类获取UUID并转换为Long类型
        config.setConfigId(SnowUtils.bigKey());
    ...
    public AjaxResult deleteConfigByIds(Long[] configIds) {
        for (Long configId : configIds) {
            ...
            if (StringUtils.equals(UserConstants.YES, config.getConfigType())) {
                return error(String.format("内置参数【%1$s】不能删除 ", config.getConfigKey()));
            }
