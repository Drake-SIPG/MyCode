...
public abstract class BaseProxyHandler {
    ...
    protected String getCache(String key, Supplier<String> init, boolean reload) {
        ...
        if (isNotBlank(value)) {
            if (value.contains("{")){
                if (ObjectUtil.equals(JSONUtil.parseObj(value).get("state"),0) && init.getClass().getName().contains("QqxyProxyHandler")){
                    value = OptionSchoolScodeUtil.getScode();
                }
                if (ObjectUtil.equals(JSONUtil.parseObj(value).get("state"),0) && init.getClass().getName().contains("CominfoProxyHandler")){
                    value = ScodeUtil.getScode();
                }

                if (ObjectUtil.equals(JSONUtil.parseObj(value).get("state"),0) && init.getClass().getName().contains("CominfoXCProxyHandler")){
                    value = ScodeUtil.getScodeXC();
                }
            }
            return value;
        }
        ...
        if (initValue.contains("{")){
            if (ObjectUtil.equals(JSONUtil.parseObj(initValue).get("state"),0) && init.getClass().getName().contains("QqxyProxyHandler")){
                initValue = OptionSchoolScodeUtil.getScode();
            }
            if (ObjectUtil.equals(JSONUtil.parseObj(value).get("state"),0) && init.getClass().getName().contains("CominfoProxyHandler")){
                initValue = ScodeUtil.getScode();
            }
            if (ObjectUtil.equals(JSONUtil.parseObj(value).get("state"),0) && init.getClass().getName().contains("CominfoXCProxyHandler")){
                initValue = ScodeUtil.getScodeXC();
            }
        }
